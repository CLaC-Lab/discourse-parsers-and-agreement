...
def main():
    ...
    edu_file_list = [os.path.join(args.edu_file_dir, fname) for fname in os.listdir(args.edu_file_dir) if fname.endswith('.txt')]
    for edu_file in edu_file_list:
            print('Parsing {}...'.format(edu_file))
            doc = create_doc_from_edu_file(edu_file, annotate_func=annotate)
            with open(os.path.join(args.output_dir, os.path.basename(edu_file) + '.edus'), 'w') as fout:
                fout.write(doc)
            print("wrote to .edus...")
